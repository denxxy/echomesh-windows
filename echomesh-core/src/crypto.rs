...
pub fn derive_public_key_impl(private_key: Vec<u8>) -> Result<IdentityKeyPair, EchoMeshError> {
    if private_key.len() != 32 { return Err(EchoMeshError::InvalidKeyLength { expected: 32, actual: private_key.len() as u32 }); }
    let bytes: [u8; 32] = private_key.as_slice().try_into().unwrap();
...
pub fn load_or_generate_identity(path: &std::path::Path) -> Result<IdentityKeyPair, EchoMeshError> {
    ...
    #[cfg(unix)] {
        ...
        let mut f = OpenOptions::new().create_new(true).write(true).mode(0o600).open(path).map_err(|e| EchoMeshError::CryptoError(format!("identity create failed: {}", e)))?;
        f.write_all(&pair.private_key).map_err(|e| EchoMeshError::CryptoError(e.to_string()))?;
    }
    #[cfg(not(unix))] { std::fs::write(path, &pair.private_key).map_err(|e| EchoMeshError::CryptoError(e.to_string()))?; }
    Ok(pair)
}
