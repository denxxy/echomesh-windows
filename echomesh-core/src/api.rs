...
impl EchoMeshClient {
    ...
    pub fn local_peer_id(&self) -> Vec<u8> { self.identity.public_key_vec() }
    pub fn local_peer_id_hex(&self) -> String { hex::encode(self.identity.public_key()) }
    ...
    pub fn connect(&self, relay_address: String, relay_public_key: Vec<u8>, secret_token_hex: Option<String>) -> Result<(), EchoMeshError> {
        if relay_public_key.len() != 32 {
            return Err(EchoMeshError::InvalidKeyLength { expected: 32, actual: relay_public_key.len() as u32 });
        }
        let server_static_key: [u8; 32] = relay_public_key.as_slice().try_into().unwrap();
        let clean_addr = relay_address.trim_start_matches("https://").trim_start_matches("http://").trim_start_matches("mesh://").trim().to_string();
        if clean_addr.is_empty() { return Err(EchoMeshError::ConnectionError("empty relay address".to_string())); }
        ...
        let connect_result = self.runtime.block_on(async {
            let mut tcp_stream = match tokio::time::timeout(connect_timeout, tokio::net::TcpStream::connect(&clean_addr)).await {
                Ok(Ok(stream)) => stream,
                Ok(Err(e)) => return Err(EchoMeshError::ConnectionError(e.to_string())),
                Err(_) => return Err(EchoMeshError::HandshakeTimeout("relay TCP connection timed out".to_string())),
            };
            let token = match secret_token_hex.as_deref() {
                Some(value) if !value.trim().is_empty() => hex::decode(value.trim()).unwrap_or_else(|_| value.trim().as_bytes().to_vec()),
                _ => crate::transport::DEFAULT_SECRET_TOKEN.to_vec(),
            };
            let hello = crate::transport::PseudoTlsBuilder::new(token, "cloudflare.com").build();
            tcp_stream.write_all(&hello).await.map_err(|e| EchoMeshError::ConnectionError(e.to_string()))?;
            tcp_stream.flush().await.map_err(|e| EchoMeshError::ConnectionError(e.to_string()))?;
            let mut noise = crate::noise::client_noise_handshake(&mut tcp_stream, &server_static_key, connect_timeout).await?;
        ...
        let (tcp_stream, noise_session) = match connect_result {
            Ok(value) => value,
            Err(error) => { self.session_mgr.disconnect(); self.set_state(NetworkState::Offline); return Err(error); }
        };
        ...
        *self.shutdown_sender.lock().unwrap() = Some(shutdown_tx);
        ...
        self.runtime.spawn(async move {
            if let Err(error) = run_relay_actor(tcp_stream, noise_session, identity, storage, listener.clone(), outbound_rx, shutdown_rx).await {
    ...
    pub fn add_contact(&self, peer_id_hex: String, name: String) -> Result<(), EchoMeshError> {
        ...
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
        self.storage.save_contact(&Contact { peer_id: peer_id.to_vec(), name, added_at: now })
    }
    ...
    pub fn send_chat_message(&self, recipient_peer_id_hex: String, text: String) -> Result<MessageRecord, EchoMeshError> {
        let recipient = crate::model::parse_peer_id(&recipient_peer_id_hex)?;
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
        let mut message = MessageRecord {
            id: format!("msg_{}_{}", now, rand::random::<u16>()), conversation_peer_id: recipient.to_vec(),
            sender_peer_id: self.identity.public_key_vec(), text: text.clone(), timestamp: now, is_outgoing: true, status: 0,
        };
        self.storage.save_message(&message)?;
        if self.session_mgr.is_transport() {
            match self.send_packet(recipient.to_vec(), text.into_bytes()) {
                Ok(()) => {
                    message.status = 1; self.storage.update_message_status(&message.id, 1)?;
                    self.listener.on_message_status_updated(message.id.clone(), DeliveryStatus::Sent);
                }
                Err(error) => {
                    message.status = 2; let _ = self.storage.update_message_status(&message.id, 2);
                    self.listener.on_message_status_updated(message.id.clone(), DeliveryStatus::Failed); return Err(error);
                }
            }
            ...
            message.status = 2; self.storage.update_message_status(&message.id, 2)?;
            self.listener.on_message_status_updated(message.id.clone(), DeliveryStatus::Failed);
        }
        Ok(message)
    }

    pub fn send_message(&self, to: String, text: String) -> Result<MessagePayload, EchoMeshError> {
        let recipient_hex = if to.to_lowercase().contains("echo") { hex::encode(crate::protocol::ECHO_SERVICE_PEER_ID) }
            else if crate::model::parse_peer_id(&to).is_ok() { to.clone() }
            else { return Err(EchoMeshError::CryptoError("recipient must be a 32-byte EchoMesh peer identity".to_string())); };
        let record = self.send_chat_message(recipient_hex, text.clone())?;
        let status = if record.status == 2 { DeliveryStatus::Failed } else { DeliveryStatus::Sent };
        Ok(MessagePayload { id: record.id, sender: "me".to_string(), recipient: to, content: text, timestamp: record.timestamp, status })
    }

    pub fn shutdown(&self) -> Result<(), EchoMeshError> { self.disconnect() }
}
...
impl EchoMeshClient {
    ...
    fn stop_active_connection(&self) {
        if let Some(tx) = self.shutdown_sender.lock().unwrap().take() { let _ = tx.send(()); }
    }
    fn spawn_local_echo(&self, recipient: [u8; 32], text: String) {
        ...
        self.runtime.spawn(async move {
            ...
            let reply = MessageRecord {
                id: format!("echo_{}_{}", now, rand::random::<u16>()), conversation_peer_id: recipient.to_vec(),
                sender_peer_id: recipient.to_vec(), text: format!("Echoed: {}", text), timestamp: now, is_outgoing: false, status: 1,
            };
