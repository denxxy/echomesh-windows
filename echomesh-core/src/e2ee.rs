...
pub fn encrypt_for_peer(recipient_public_key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, EchoMeshError> {
    if recipient_public_key.len() != 32 { return Err(EchoMeshError::InvalidKeyLength { expected: 32, actual: recipient_public_key.len() as u32 }); }
    ...
    let mut out = Vec::with_capacity(HEADER_LEN + ciphertext.len()); out.push(VERSION); out.extend_from_slice(ephemeral_public.as_bytes()); out.extend_from_slice(&nonce_bytes); out.extend_from_slice(&ciphertext); Ok(out)
}

pub fn decrypt_from_peer(local_private_key: &[u8], envelope: &[u8]) -> Result<Vec<u8>, EchoMeshError> {
    if local_private_key.len() != 32 { return Err(EchoMeshError::InvalidKeyLength { expected: 32, actual: local_private_key.len() as u32 }); }
    if envelope.len() <= HEADER_LEN || envelope[0] != VERSION { return Err(EchoMeshError::CryptoError("invalid E2EE envelope".into())); }
    ...
    let hk = Hkdf::<Sha256>::new(None, shared.as_bytes());
    let mut key = [0u8; 32]; hk.expand(HKDF_INFO, &mut key).map_err(|_| EchoMeshError::CryptoError("HKDF expansion failed".into()))?;
    let cipher = ChaCha20Poly1305::new_from_slice(&key).map_err(|_| EchoMeshError::CryptoError("invalid AEAD key".into()))?;
